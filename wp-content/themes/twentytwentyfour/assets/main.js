...
hm.onclick = function( ev ) {
	lastX = ev.pageX;
	lastY = ev.pageY;
	data.push({ x:lastX, y:lastY, value: 5 });
};
...
heatmap = h337.create({ container:hm, radius:60 });
...
async function htmlToSvg(element) {
    // Ensure this function converts HTML to SVG properly
    // Placeholder implementation, you need to use a proper library or logic for this
    let svg = document.createElementNS("http://www.w3.org/2000/svg", "svg");
    svg.innerHTML = element.innerHTML;
    return svg;
}

function htmlToSvg(element) {
    return new Promise((resolve, reject) => {
        ...
        // Create a foreignObject element to hold HTML content
        let foreignObject = document.createElementNS("http://www.w3.org/2000/svg", "foreignObject");
        foreignObject.setAttribute("width", "100%");
        foreignObject.setAttribute("height", "100%");
        
        // Serialize the HTML content
        let htmlString = new XMLSerializer().serializeToString(element);
        let htmlBlob = new Blob([htmlString], {type: 'application/xhtml+xml'});
        ...
        iframe.onload = function() {
            // Transfer the HTML content to the foreignObject
            let doc = iframe.contentDocument || iframe.contentWindow.document;
            foreignObject.appendChild(doc.body.firstChild);
            svg.appendChild(foreignObject);
            URL.revokeObjectURL(htmlUrl); // Clean up the object URL
            resolve(svg);
        };
        iframe.src = htmlUrl;
        console.log(htmlUrl)
...
function svgToCanvas(svgElement) {
    return new Promise((resolve, reject) => {
        ...
        let svg64 = utf8_to_b64(svgString);
...
// Example usage
// Suppose you have an SVG element
...
window.onbeforeunload = function(event){
    htmlToSvg(document.querySelector('.wp-embed-responsive')).then((svg) => {
        console.log(svg);
        document.body.appendChild(svg); // Append SVG to the body or another element
    }).catch((error) => {
        console.error('Error converting HTML to SVG:', error);
    });
    let svgElements = document.querySelectorAll('svg'); // or create a new SVG element
    let lastSvgElement = svgElements[svgElements.length - 1];
    svgToCanvas(lastSvgElement).then((canvas) => {
        document.body.appendChild(canvas); // Append the canvas to the body or another element
        console.log(canvas.toDataURL()); // Get the data URL of the canvas
    }).catch((error) => {
